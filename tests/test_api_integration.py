"""Integration tests for the live Perplexity API.

Contains the real-API classes (``TestPerplexityAPIIntegration``,
``TestAPIErrorHandling``) that exercise the live Perplexity API and are
skipped unless ``RUN_REAL_API_TESTS=1``.  The hermetic loopback-harness
class lives in ``tests/test_api_protocol_integration.py``.
"""
