...
class TestInlineFilePath:
    ...
    def test_inline_file_path_in_query(self, tmp_path, monkeypatch):
...
Status: Complete""",
        ...
        patch_query_deps(
            ...
            load_token_optional=lambda _tm, _logger: ("test-token", None),
        )
        with patch("perplexity_cli.attachments.AttachmentUploader", return_value=mock_uploader):
            # Mock style manager
            mock_sm = Mock()
            ...
            result = runner.invoke(query, ["--no-stream", "--attach", str(test_file), query_text])
            ...
            assert attachments[0].startswith("https://ppl-ai-file-upload.s3.amazonaws.com/")
