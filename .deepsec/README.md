...
Currently configured project: `perplexity-cli` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/perplexity-cli/SETUP.md` to fill in
   `data/perplexity-cli/INFO.md`.
...
`--project-id perplexity-cli` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/perplexity-cli/`.
