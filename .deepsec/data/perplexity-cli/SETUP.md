# Agent setup for `perplexity-cli`

This is a deepsec scanning workspace. Project `perplexity-cli` was just registered
(target: `..`). Setup is incomplete — `data/perplexity-cli/INFO.md`
...
2. **Fill in `data/perplexity-cli/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id perplexity-cli
pnpm deepsec process --project-id perplexity-cli
```
